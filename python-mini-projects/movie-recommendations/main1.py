...
# the next step is to get the title of the movie that the user currently likes. then we will find the index of that movie. after that, we will access the row corresponding to this movie in the similarity matrix. thus, we will get the similarity scores of all other movies from the current movie. then we will enumerate through all the similarity scores of that movie to make a tuple of movie index and similarity score . this will convert a row of similarity scores like this- [1 0.5 0.2 0.9] to this- [(0,1) (1,0.5) (2,0.2) (3,0.9)]. here each term is in this form- (movie index, similarity score)
