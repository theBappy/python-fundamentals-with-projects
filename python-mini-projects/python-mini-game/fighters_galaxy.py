...
WINNING_SOUND = pygame.mixer.Sound(os.path.join("Assets", "winning.mp3"))
...
def show_play_again():
    WIN.fill(BLACK)
    text = WINNER_FONT.render("Play Again?", 1, WHITE)
    WIN.blit(text, (WIDTH/2 - text.get_width()/2, HEIGHT//2 - 100))

    # Buttons
    yes_button = pygame.Rect(WIDTH//2 - 150, HEIGHT//2, 120, 60)
    no_button = pygame.Rect(WIDTH//2 + 30, HEIGHT//2, 120, 60)

    pygame.draw.rect(WIN, GREEN, yes_button)
    pygame.draw.rect(WIN, RED, no_button)

    yes_text = BUTTON_FONT.render("YES", 1, BLACK)
    no_text = BUTTON_FONT.render("NO", 1, BLACK)

    WIN.blit(yes_text, (yes_button.x + yes_button.width//2 - yes_text.get_width()//2,
...
def main():
    ...
    while run:
        ...
        if winner_text != "":
            ...
            return show_play_again()  # Ask if player wants to play again
...
if __name__ == "__main__":
    play = True
    while play:
        play = main()  # main() returns True if Yes clicked, False if No or quit
