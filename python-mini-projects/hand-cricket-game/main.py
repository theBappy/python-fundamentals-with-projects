...
def main():
    ...
    try:
        ...
        if toss_winner == 1:
            ...
            player1_choice = input("Player 1, choose 1 to bar first, 2 to bowl first: ")
            player2_choice = "1" if player1_choice == "2" else "2"
        else:
            print("Player 2 won the toss!")
            player2_choice = input("Player 2, choose 1 to bat first or 2 to bowl first: ")
    ...
    for over in range(overs):
        ...
        if player1_choice == "1":
            player1_score, player1_wickets = user_turn(
                player2_score, player1_wickets, "1", over
            )
...
def user_turn(player_score, player_wickets, player_choice, over):
    ...
    while balls < 6 and player_wickets > 0:
        if player_choice == "1":
            player_runs = int(
                input(
                    f"Over {over+1}, Ball {balls+1}: Enter your {'shot'} (1-6): "
                )
            )
        ...
        print(f"You choose {player_runs}, Opponent choose {opponent_runs}")
        ...
        if player_choice == "1" and player_runs == opponent_runs:
            ...
            player_choice == "2"
            and opponent_choice == "2" and player_runs == opponent_runs
        ):
